 ...
 * 
 *         A deadlock *could* arise if several Connection threads try to obtain both
 *         a server lock and a board lock for a single operation, however these
 *         operations are independent and don't ever attempt to acquire both
...
public class WhiteboardServer {
	...
	public synchronized int removeConnection() {
		numConnections--;
		return numConnections;
	}
	...
	private void handleConnection(Socket socket) throws IOException {
		(new Thread(new User(socket, this)))
				.start();
	}
	...
	/**
	 * Start a WhiteboardServer running on the specified port, with either a
